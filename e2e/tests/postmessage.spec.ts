...
test('Parent → Child via postMessage with allowed "*"', async ({
  page,
  baseURL,
}) => {
  ...
  await expect
    .poll(async () =>
      page.frames().some((f) => f.url().includes("/e2e/pages/child.html")),
    )
    .toBe(true);
  const childFrame = page
    .frames()
    .find((f) => f.url().includes("/e2e/pages/child.html"))!;
  await childFrame.waitForFunction(
    () => (window as unknown as TestWin).__fluxa_ready__ === true,
  );
  await childFrame.evaluate(() =>
    (window as unknown as TestWin).observe("ping"),
  );
  await page.evaluate(() =>
    (window as unknown as TestWin).emit("ping", { n: 1 }),
  );
  await expect
    .poll(async () =>
      childFrame.evaluate(() =>
        (window as unknown as TestWin).getCount("ping"),
      ),
    )
    .toBe(1);
});

test('Child → Parent via postMessage with allowed "*"', async ({
  page,
  baseURL,
}) => {
  await page.goto(`${baseURL}/e2e/pages/host.html?allow=*`);
  ...
  await childFrame.evaluate(() =>
    (window as unknown as TestWin).emit("pong", { n: 2 }),
  );
  await expect
    .poll(async () =>
      page.evaluate(() => (window as unknown as TestWin).getCount("pong")),
    )
    .toBe(1);
});

test("Blocked by allowedOrigins mismatch (child on different origin)", async ({
  page,
}) => {
  ...
  await page.goto(
    `${host}/e2e/pages/host.html?allow=same&child_origin=${encodeURIComponent(child)}&child_allow=*`,
  );
  await page.waitForFunction(
    () => (window as unknown as TestWin).__fluxa_ready__ === true,
  );
  ...
  await frameEl.waitForElementState("stable");
  await expect
    .poll(async () => page.frames().some((f) => f.url().startsWith(child)))
    .toBe(true);
  ...
  await childFrame.evaluate(() =>
    (window as unknown as TestWin).emit("x", { n: 3 }),
  );
  // Give it a moment and assert still zero
  await page.waitForTimeout(200);
  expect(
    await page.evaluate(() => (window as unknown as TestWin).getCount("x")),
  ).toBe(0);
  ...
  await expect
    .poll(async () =>
      childFrame.evaluate(() => (window as unknown as TestWin).getCount("y")),
    )
    .toBe(1);
});

test("unregisterFramePeer stops Parent → Child delivery", async ({
  page,
  baseURL,
}) => {
  await page.goto(`${baseURL}/e2e/pages/host.html?allow=*`);
  ...
  await expect
    .poll(async () =>
      childFrame.evaluate(() => (window as unknown as TestWin).getCount("z")),
    )
    .toBe(1);
  ...
  expect(
    await childFrame.evaluate(() =>
      (window as unknown as TestWin).getCount("z"),
    ),
  ).toBe(1);
});
