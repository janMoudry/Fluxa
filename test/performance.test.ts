...
describe('Performance & Reliability', () => {
  ...
  describe('Fluxa memory transport burst', () => {
    it('delivers 5k local events without loss and under time limit', () => {
      ...
      const a = new Fluxa<E>({ propagation: { memory: true }, context: { id: 'A' } });
      const N = 5_000;
      let got = 0;
      let last = -1;
      a.on('ping', (d) => {
  ...
  describe('BroadcastChannel (tab) burst', () => {
    ...
    it('delivers 2k events cross-tab without loss and under time limit', () => {
      ...
      const a = new Fluxa<E>({ propagation: { memory: false, tab: true }, context: { id: 'A' }, tab: { channel: 'perf-ch' } });
      const b = new Fluxa<E>({ propagation: { memory: false, tab: true }, context: { id: 'B' }, tab: { channel: 'perf-ch' } });
      const N = 2_000;
      let got = 0;
      let last = -1;
      b.on('hello', (d) => {
  ...
  describe('PostMessage (frame) burst', () => {
    ...
    it('delivers 1k events cross-frame without loss and under time limit', () => {
      ...
      const a = new Fluxa<E>({ propagation: { frame: true }, context: { id: 'A' }, frame: { allowedOrigins: ['*'], channel: 'perf-pm' } });
      (globalThis as unknown as { window?: Window }).window = winB as unknown as Window;
      const b = new Fluxa<E>({ propagation: { frame: true }, context: { id: 'B' }, frame: { allowedOrigins: ['*'], channel: 'perf-pm' } });
      (globalThis as unknown as { window?: Window }).window = winA as unknown as Window;
