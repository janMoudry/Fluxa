...
describe('Meta path and loop prevention', () => {
  ...
  it('appends context id on receive and prevents re-entry loops', () => {
    const a = new Fluxa<Events>({ propagation: { memory: true, tab: true }, context: { id: 'A' }, tab: { channel: 'fluxa' } });
    const b = new Fluxa<Events>({ propagation: { memory: true, tab: true }, context: { id: 'B' }, tab: { channel: 'fluxa' } });
