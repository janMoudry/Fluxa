import type { FluxaEventMap, FluxaHandler, FluxaFilter } from "./types";
import type { Fluxa } from "./Fluxa";

type StripPrefix<K extends string, P extends string> = K extends `${P}:${infer R}`
  ? R
  : never;
type PrefixedKeys<E extends FluxaEventMap, P extends string> = Extract<
  keyof E,
  `${P}:${string}`
> extends infer Keys
  ? Keys extends string
...
export class Scope<Events extends FluxaEventMap, P extends string> {
  ...
  on<K extends keyof EventsForPrefix<Events, P>>(
    ...
    const full = `${this.prefix}:${String(event)}` as PrefixedKeys<
      ...
      P
    >;
    return this.bus.on(full, handler as unknown as FluxaHandler<Events[typeof full]>, filter);
  }
  ...
  off<K extends keyof EventsForPrefix<Events, P>>(
    ...
    const full = `${this.prefix}:${String(event)}` as PrefixedKeys<
      ...
      P
    >;
    this.bus.off(full, handler as unknown as FluxaHandler<Events[typeof full]>);
  }
}
